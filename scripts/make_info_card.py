...
ROWS = [
    ("host",),
    ("kv", "Now", "TA & Lab Infra Eng. @ INU"),
    ("kv", "Prev", "AI Engineer @ HyvenTechJo"),
    ("kv", "Also", "AI Automation Trainee @ Robotna"),
    ("kv", "Edu", "B.Sc. Data Science & AI, INU '25"),
    ("gap",),
    ("sec", "Stack"),
    ("kv", "AI / ML", "LangChain, LangGraph, RAG"),
    ("kv", "ML / Data", "TensorFlow, Scikit-learn, SQL"),
    ("kv", "Vision/NLP", "Computer Vision (CNN), NLP"),
    ("kv", "Backend", "FastAPI, GraphQL, Docker, K8s"),
    ("kv", "Tools", "n8n, Supabase/pgvector, Notion"),
    ("gap",),
    ("sec", "Highlights"),
    ("bul", "RAG + multi-agent systems - up to 30% accuracy"),
    ("bul", "CNN waste-classifier -- 95% accuracy"),
]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">abdk@github: ~$ neofetch</text>')
...
for i, row in enumerate(ROWS):
    ...
    if kind == "host":
        inner = (f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
                 f'<tspan fill="{GREEN}">abdk</tspan><tspan fill="{MUTED}">@</tspan>'
